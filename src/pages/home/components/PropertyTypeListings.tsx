...
interface PropertyTypeListingsProps {
  propertyType: string;
  title: string;
}
...
export default function PropertyTypeListings({ propertyType, title }: PropertyTypeListingsProps) {
  ...
  const fetchProperties = async () => {
    try {
      ...
      if (propertiesError) {
        console.error('PropertyTypeListings: Properties error:', propertiesError);
      ...
      const propertiesWithImages = await Promise.all(
        (propertiesData || []).map(async (property: Property) => {
          try {
            ...
            console.error('PropertyTypeListings: Image error for property', property.id, error);
